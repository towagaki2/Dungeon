...
struct DungeonMap_RL
{
	...
	size_t divCountRand = 3; //マップの区分け数加算
...
int rogueLikeMapMake(DungeonMap_RL* const dng, T& maprl)
{
	...
	for (size_t i = 0; i < maprl.size(); ++i)
		for (size_t j = 0; j < maprl[i].size(); ++j)
			maprl[i][j].mapData = 1;
	dng->mapDivCount = dng->divCountMin + (size_t)GetRand(Rand) % (int)dng->divCountRand; //マップの区分け数 (部屋の個数) 0~nまでの部屋ID
	...
	for (size_t i = 1; i < dng->mapDivCount; ++i) {
		...
		std::uniform_int_distribution<> GetRand2(0, (int)i - 1);        // [0, 99] 範囲の一様乱数
		//今まで作った区分けをランダムに指定(指定した区域をさらに区分けする)
		divAfter = (size_t)(GetRand2(Rand));
		...
		//count軸の設定
		dng->mapDiv[i][count] = dng->mapDiv[divAfter][count + 2] + (dng->mapDiv[divAfter][count] - dng->mapDiv[divAfter][count + 2]) / 3 + (size_t)GetRand(Rand) % (int)((dng->mapDiv[divAfter][count] - dng->mapDiv[divAfter][count + 2]) / 3); //0.軸の右端(iR)の座標(divAfter*R/3~2divAfter*R/3)
	...
	for (size_t i = 0; i < dng->mapDivCount; ++i) {//区分け
		...
		dng->mapRoom[i][3] = dng->mapDiv[i][3]; //区分けY始点をマップY始点へ代入
												...
												//X座標の部屋の長さを指定
		dng->mapRoom[i][0] = dng->mapDiv[i][2] + dng->divCountRand + (size_t)GetRand(Rand) % (int)dng->roomLengthRandX;
		...
		dng->mapRoom[i][1] = dng->mapDiv[i][3] + dng->roomLengthMinY + (size_t)GetRand(Rand) % (int)dng->roomLengthRandY;
		...
		size_t l;
		size_t m;
		if (((int)dng->mapDiv[i][0] - (int)dng->mapRoom[i][0] - 5) != 0) {
			l = (size_t)GetRand(Rand) % ((int)dng->mapDiv[i][0] - (int)dng->mapRoom[i][0] - 5) + 2;
		}
		else
		{
			l = 2;
		}
		if (((int)dng->mapDiv[i][1] - (int)dng->mapRoom[i][1] - 5) != 0) {
			m = (size_t)GetRand(Rand) % ((int)dng->mapDiv[i][1] - (int)dng->mapRoom[i][1] - 5) + 2;
		}
		else
		{
			m = 2;
	...
	*/
	...
	for (size_t roomBefore = 0; roomBefore < dng->mapDivCount; ++roomBefore) {
		...
		case RL_COUNT_X:
			dng->mapRoad[roomBefore][2] = (size_t)GetRand(Rand) % (int)(dng->mapRoom[roomBefore][1] - dng->mapRoom[roomBefore][3]) - 2; //前側の通路の位置
			dng->mapRoad[roomBefore][3] = (size_t)GetRand(Rand) % (int)(dng->mapRoom[roomAfter][1] - dng->mapRoom[roomAfter][3] )- 2; //後側の通路の位置
																															   //前の通路
		...
		case RL_COUNT_Y:
			dng->mapRoad[roomBefore][2] = (size_t)GetRand(Rand) % (int)(dng->mapRoom[roomBefore][0] - dng->mapRoom[roomBefore][2]) - 2; //前側の通路の位置
			dng->mapRoad[roomBefore][3] = (size_t)GetRand(Rand) % (int)(dng->mapRoom[roomAfter][0] - dng->mapRoom[roomAfter][2]) - 2; //後側の通路の位置
																															   //前の通路
